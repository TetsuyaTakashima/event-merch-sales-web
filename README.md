...
- 確認用のテスト販売アカウント。本番売上とは分離されたテスト環境でイベント、商品、販売データを確認
- 画面上からのデータバックアップと復元
...
`テスト販売` 権限のユーザーは、本番用の `app_state` 行ではなく `sandbox:<ユーザーID>` の確認用データだけを利用します。本番の売上・在庫・イベントは表示されず、テスト環境内でイベント作成、商品登録、販売登録、集計確認を行えます。
