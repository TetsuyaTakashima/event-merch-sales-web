...
- 確認用のテスト販売アカウント。本番売上とは分離されたテスト環境でイベント、商品、販売データを確認。テストイベントは作成から3日後に自動削除
- 画面上からのデータバックアップと復元
...
`テスト販売` 権限のユーザーは、本番用の `app_state` 行ではなく `sandbox:<ユーザーID>` の確認用データだけを利用します。本番の売上・在庫・イベントは表示されず、テスト環境内でイベント作成、商品登録、販売登録、集計確認を行えます。テスト環境で作成したイベントは、イベント作成から3日後にイベントごと自動削除され、関連する在庫・販売履歴・商品紐づきもテスト環境内だけで整理されます。
...
- `supabase/add-tester-role.sql`: 既存Supabase環境に `テスト販売` 権限を追加
- `supabase/add-tester-event-ttl.sql`: テスト販売用イベントを作成から3日後に自動削除するCron設定
